...
sel311c_met = """BROKEN BOW 11S-02                 Date: 07/21/16    Time: 16:51:54.489
...
sel351delta_met = """BROKEN BOW 11T1L SEL-351-6        Date: 07/21/16    Time: 16:56:07.849
...
sel351delta_met_values = {
    'RID': 'BROKEN BOW 11T1L SEL-351-6',
...
sel421_met = """BROKEN BOW 11S-08 SEL-421 NON-PILOT        Date: 07/21/2016  Time: 16:54:23.886
...
sel421_met_values = {
    'RID': 'BROKEN BOW 11S-08 SEL-421 NON-PILOT',
...
def assert_round_equals(n1, n2, places=3):
    if isinstance(n1, dict):
        assert list(n1) == list(n2)
        for k in n2:
            assert_round_equals(n1[k], n2[k], places)
        return
    try:
        # Complex numbers test real and imaginary parts
        try:
            assert round(n1.real, places) == round(n2.real, places)
            assert round(n1.imag, places) == round(n2.imag, places)
        except AttributeError:
            # Round numbers that can be rounded
            assert round(n1, places) == round(n2, places)
    except TypeError:
        # Types that can't be rounded are checked for equivalence
        assert n1 == n2


@pytest.mark.parametrize("tc, met_text, met_values_dict",
                         [(su.RelaySEL311C().met, sel311c_met,
                           sel311c_met_values),
                          (su.RelaySEL421().met, sel421_met,
                           sel421_met_values),
                          (su.RelaySEL351Delta().met, sel351delta_met,
                           sel351delta_met_values)])
def test_parse_met(tc, met_text, met_values_dict):
    assert tc.match(met_text) is True
    tc.read(met_text)
    for k, v in met_values_dict.items():
        assert_round_equals(tc.data[k], v)


@pytest.mark.parametrize('q, d_in, d_expected',
                         [('IA',
                           {'IA_MAG': 1., 'IA_ANG': 0.},
                           {'IA_MAG': 1., 'IA_ANG': 0., 'IA': 1.0}),
                          ('VA',
                           {'VA_MAG': 1., 'VA_ANG': 180.},
                           {'VA_MAG': 1., 'VA_ANG': 180., 'VA': -1.0}),
                          ('IA',
                           {'IA_MAG': 1., 'IA_ANG': 90.},
                           {'IA_MAG': 1., 'IA_ANG': 90., 'IA': 1.j})
                          ])
def test_mag_ang_to_complex(q, d_in, d_expected):
    su._mag_ang_to_complex(q, d_in)
    assert_round_equals(d_in, d_expected, places=7)


@pytest.mark.parametrize('q, d_in, d_expected',
                         [('TEST',
                           {'TEST': '  string  ', 'IA_ANG': 0.},
                           {'TEST': 'string', 'IA_ANG': 0.}),
                          ('TEST',
                           {'TEST': 'string', 'IA_ANG': 0.},
                           {'TEST': 'string', 'IA_ANG': 0.}),
                          ('TEST',
                           {'TEST': '  str  ing  ', 'IA_ANG': 0.},
                           {'TEST': 'str  ing', 'IA_ANG': 0.})
                          ])
def test_strip_string(q, d_in, d_expected):
    su._strip_string(q, d_in)
    assert d_in == d_expected
